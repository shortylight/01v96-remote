...
var remoteApp = {
	...
	generatePage: function() {
		...
		effectHeadHTML = '<div class="fader-biglabel">Channel</div>\
		<div>\
		<div class="fader-select" id="1" style="width:20% ; float: left; margin:15px"> &lt;  &lt; </div>\
		<div class="fader-select" id="2" style="width:20% ; float: right; margin:15px"> &gt; &gt; </div>\
		</div>';
	...
	updateEffectsData: function(num) {
		...
		$('.tabcontent[data-tab="effects"] .fader-select').first().html('&lt ' + (num > 1 ? (num -1) : 1) + ' &lt;');
		$('.tabcontent[data-tab="effects"] .fader-select').eq(1).attr('id',(num < this.config.channelCount ? (1 + (1*num)) : this.config.channelCount));
		$('.tabcontent[data-tab="effects"] .fader-select').eq(1).html('&gt; ' + (num < this.config.channelCount ? (1 + (1*num)) : this.config.channelCount) + ' &gt;');
		//$('.fader-label').html(num);
